...
final class ByteBufferToOutboundDecoder: ChannelInboundHandler, Loggable {
  ...
  private static func decodeSetName (buffer: inout ByteBuffer) -> Outbound? {
    guard let personType = buffer.readPersonType() else {
      log.error("The 'setName' message doesn't have 'person type' field")
      return nil
    }
    guard let name = buffer.readString() else {
      log.error("The 'setName' message doesn't have 'name' field")
      return nil
    }
  ...
  private static func decodeSetScore (buffer: inout ByteBuffer) -> Outbound? {
    guard let personType = buffer.readPersonType() else {
      log.error("The 'setScore' message doesn't have 'person type' field")
      return nil
    }
    guard let score = buffer.readUInt8() else {
      log.error("The 'setScore' message doesn't have 'score' field")
      return nil
    }
  ...
  private static func decodeSetCard (buffer: inout ByteBuffer) -> Outbound? {
    guard let personType = buffer.readPersonType() else {
      log.error("The 'setCard' message doesn't have 'person type' field")
      return nil
    }
    guard let statusCard = buffer.readStatusCard() else {
      log.error("The 'setCard' message doesn't have 'status card' field")
      return nil
    }
  ...
  private static func decodeSetPriority (buffer: inout ByteBuffer) -> Outbound? {
    guard let personType = buffer.readPersonType() else {
      log.error("The 'setPriority' message doesn't have 'person type' field")
      return nil
    }
  ...
  private static func decodeSetPeriod (buffer: inout ByteBuffer) -> Outbound? {
    guard let period = buffer.readUInt8() else {
      log.error("The 'setPeriod' message doesn't have 'period' field")
      return nil
    }
  ...
  private static func decodeSetWeapon (buffer: inout ByteBuffer) -> Outbound? {
    guard let weapon = buffer.readWeapon() else {
      log.error("The 'setWeapon' message doesn't have 'weapon' field")
      return nil
    }
  ...
  private static func decodeSetTimer (buffer: inout ByteBuffer) -> Outbound? {
    guard let time = buffer.readUInt32() else {
      log.error("The 'setTimer' message doesn't have 'time' field")
      return nil
    }
    guard let mode = buffer.readTimerMode() else {
      log.error("The 'setTimer' message doesn't have 'mode' field")
      return nil
    }
  ...
  private static func decodeStartTimer (buffer: inout ByteBuffer) -> Outbound? {
    guard let state = buffer.readTimerState() else {
      log.error("The 'startTimer' message doesn't have 'state' field")
      return nil
    }
  ...
  private static func decodeVisibility (buffer: inout ByteBuffer) -> Outbound? {
    guard let video = buffer.readUInt8() else {
      log.error("The 'visibility' message doesn't have 'video' field")
      return nil
    }
    guard let photo = buffer.readUInt8() else {
      log.error("The 'visibility' message doesn't have 'photo' field")
      return nil
    }
    guard let passive = buffer.readUInt8() else {
      log.error("The 'visibility' message doesn't have 'passive' field")
      return nil
    }
    guard let country = buffer.readUInt8() else {
      log.error("The 'visibility' message doesn't have 'country' field")
      return nil
    }
    return .visibility(
        video: video == 0,
  ...
  private static func decodeVideoCounters (buffer: inout ByteBuffer) -> Outbound? {
    guard let left = buffer.readUInt8() else {
      log.error("The 'videoCounters' message doesn't have 'left' field")
      return nil
    }
    guard let right = buffer.readUInt8() else {
      log.error("The 'videoCounters' message doesn't have 'right' field")
      return nil
    }
  ...
  private static func decodePassiveTimer (buffer: inout ByteBuffer) -> Outbound? {
    guard let show = buffer.readUInt8() else {
      log.error("The 'passiveTimer' message doesn't have 'show' field")
      return nil
    }
    guard let locked = buffer.readUInt8() else {
      log.error("The 'passiveTimer' message doesn't have 'locked' field")
      return nil
    }
    guard let timer = buffer.readUInt32() else {
      log.error("The 'passiveTimer' message doesn't have 'timer' field")
      return nil
    }
  ...
  private static func decodeSetDefaultTime (buffer: inout ByteBuffer) -> Outbound? {
    guard let time = buffer.readUInt32() else {
      log.error("The 'setDefaultTime' message doesn't have 'time' field")
      return nil
    }
  ...
  private static func decodeSetCompetition (buffer: inout ByteBuffer) -> Outbound? {
    guard let name = buffer.readString() else {
      log.error("The 'setCompetition' message doesn't have 'name' field")
      return nil
    }
  ...
  private static func decodeVideoRoutes (buffer: inout ByteBuffer) -> Outbound? {
    guard let camerasCount = buffer.readUInt8() else {
      log.error("The 'videoRoutes' message doesn't have 'cameras count' field")
      return nil
    }
    ...
    for index in 0..<camerasCount {
      guard let camera = buffer.readCamera() else {
        log.error("The 'videoRoutes' message doesn't have 'camera[{}]' field", index)
        return nil
      }
  ...
  private static func decodeLoadFile (buffer: inout ByteBuffer) -> Outbound? {
    guard let name = buffer.readString() else {
      log.error("The 'loadFile' message doesn't have 'name' field")
      return nil
    }
  ...
  private static func decodePlayer (buffer: inout ByteBuffer) -> Outbound? {
    guard let speed = buffer.readUInt8() else {
      log.error("The 'player' message doesn't have 'speed' field")
      return nil
    }
    guard let recordMode = buffer.readRecordMode() else {
      log.error("The 'player' message doesn't have 'recordMode' field")
      return nil
    }
    guard let timestamp = buffer.readUInt32() else {
      log.error("The 'player' message doesn't have 'timestamp' field")
      return nil
    }
  ...
  private static func decodeRecord (buffer: inout ByteBuffer) -> Outbound? {
    guard let recordMode = buffer.readRecordMode() else {
      log.error("The 'record' message doesn't have 'record mode' field")
      return nil
    }
  ...
  private static func decodeEthernetNextOrPrevious (buffer: inout ByteBuffer) -> Outbound? {
    guard let next = buffer.readUInt8() else {
      log.error("The 'ethernetNextOrPrevious' message doesn't have 'next' field")
      return nil
    }
  ...
  private static func decodeGenericResponse (buffer: inout ByteBuffer) -> Outbound? {
    guard let request = buffer.readUInt8() else {
      log.error("The 'genericResponse' message doesn't have 'request' field")
      return nil
    }
  ...
  private static func decodeAuthenticate (buffer: inout ByteBuffer) -> Outbound? {
    guard let deviceType = buffer.readDeviceType() else {
      log.error("The 'authenticate' message doesn't have 'device' field")
      return nil
    }
    guard let code = buffer.readBytes() else {
      log.error("The 'authenticate' message doesn't have 'code' field")
      return nil
    }
    guard let name = buffer.readString() else {
      log.error("The 'authenticate' message doesn't have 'name' field")
      return nil
    }
    guard let version = buffer.readUInt8() else {
      log.error("The 'authenticate' message doesn't have 'version' field")
      return nil
    }
  ...
  public func errorCaught (context: ChannelHandlerContext, error: Error) {
    log.error("during channel handling - {}", error)
