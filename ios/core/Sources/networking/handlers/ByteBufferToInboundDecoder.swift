...
final class ByteBufferToInboundDecoder: ChannelInboundHandler, Loggable {
  ...
  private static func decodeBroadcast (buffer: inout ByteBuffer) -> Inbound? {
    guard let weapon = buffer.readWeapon() else {
      log.error("The 'broadcast' message doesn't have 'weapon' field")
      return nil
    }
    guard let leftFlag = buffer.readFlagState() else {
      log.error("The 'broadcast' message doesn't have 'leftFlag' field")
      return nil
    }
    guard let rightFlag = buffer.readFlagState() else {
      log.error("The 'broadcast' message doesn't have 'rightFlag' field")
      return nil
    }
    guard let timer = buffer.readUInt32() else {
      log.error("The 'broadcast' message doesn't have 'timer' field")
      return nil
    }
    guard let timerState = buffer.readTimerState() else {
      log.error("The 'broadcast' message doesn't have 'timerState' field")
      return nil
    }
  ...
  private static func decodeDeviceList (buffer: inout ByteBuffer) -> Inbound? {
    guard let devicesCount = buffer.readUInt8() else {
      log.error("The 'decodeDeviceList' message doesn't have 'count' field")
      return nil
    }
    ...
    for index in 0..<devicesCount {
      guard let device = buffer.readDevice() else {
        log.error("The 'decodeDeviceList' message doesn't have 'device[{}]' field", index)
        return nil
      }
  ...
  private static func decodeEthernetDisplay (buffer: inout ByteBuffer) -> Inbound? {
    guard let period = buffer.readUInt8() else {
      log.error("The 'ethernetDisplay' message doesn't have 'period' field")
      return nil
    }
    guard let time = buffer.readUInt32() else {
      log.error("The 'ethernetDisplay' message doesn't have 'time' field")
      return nil
    }
    guard let leftSide = buffer.readSide() else {
      log.error("The 'ethernetDisplay' message doesn't have 'left side' field")
      return nil
    }
    guard let rightSide = buffer.readSide() else {
      log.error("The 'ethernetDisplay' message doesn't have 'right side' field")
      return nil
    }
  ...
  private static func decodeFightResult (buffer: inout ByteBuffer) -> Inbound? {
    guard let decision = buffer.readDecision() else {
      log.error("The 'fightResult' message doesn't have 'result' field")
      return nil
    }
  ...
  private static func decodeVideoReady (buffer: inout ByteBuffer) -> Inbound? {
    guard let name = buffer.readString() else {
      log.error("The 'videoReady' message doesn't have 'name' field")
      return nil
    }
  ...
  private static func decodeAuthentication (buffer: inout ByteBuffer) -> Inbound? {
    guard let status = buffer.readAuthenticationStatus() else {
      log.error("The 'authentication' message doesn't have 'status' field")
      return nil
    }
